...
TEMP_DIR=$(mktemp -d)
FINAL_ZIP=$PWD/'warm-up-lex-lambda.zip'
...
zip -g $FINAL_ZIP -r . --exclude 'scripts/*' 'venv/*' '**/__pycache__/*' .DS_Store README.md .gitignore requirements.txt requirements-dev.txt '*.zip' package.sh '*.mp3'
rm -rf $WORK_DIR